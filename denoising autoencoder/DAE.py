...
def mkModel(x):
    ...
    conv1_pool = normalization.batch_normalization(conv1_pool)

    conv2 = conv_layer(conv1_pool,filters["conv2/deconv2"],name='conv2')
    conv2_pool = max_pool_2x2(conv2,name='conv2_pool')
    conv2_pool = normalization.batch_normalization(conv2_pool)

    encode = full_layer(tf.reshape(conv2_pool,[-1,7*7*8]),20)
    encode = normalization.batch_normalization(encode)

    decode = full_layer(encode,7*7*8)
    decode = normalization.batch_normalization(decode)
    ...
    deconv1 = normalization.batch_normalization(deconv1)
    ...
    decoder = tf.nn.relu(deconv_layer(unpool2,filters["conv1/deconv1"],tf.shape(x_image),name='decode2'))
    print("decode2:{}".format(decode.shape))

    return x_image,decoder
...
with tf.Session() as sess:
    ...
    for i in range(5000):
        ...
        x_noise = Noise(x_)
        if i % 10 == 0:
            print('In {} step,cost: {}'.format(i,sess.run(cost,feed_dict={x:x_noise.GaussianNoise(0),y_i:x_})))
        sess.run(train_step,feed_dict={x:x_noise.GaussianNoise(0),y_i:x_})
